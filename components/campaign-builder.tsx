...
export default function CampaignBuilder({ mode, campaignId }: CampaignBuilderProps) {
    ...
    fetch("/api/automations", { cache: "no-store" })
      ...
      .then((payload) => {
        ...
        setFollowPromptButtonLabel(
          c.followPromptButtonLabel ?? "ya te sigo"
        );
  ...
  async function handleSubmit(activeValue: boolean) {
    ...
    if (!selectedAccountId) return setError("Primero conecta una cuenta de Instagram.");
    ...
    if (matchMode === "specific" && keywords.length === 0)
      return setError("Agrega al menos una palabra, o cambia a cualquier palabra.");
    if (!dmMessage.trim()) return setError("Agrega el DM con el enlace.");
    if (openingDmEnabled && (!openingDmMessage.trim() || !openingDmButtonLabel.trim()))
      return setError("Tu DM de bienvenida necesita un mensaje y un texto de botón.");
    ...
    const payload = {
      ...
      followPromptButtonLabel: requireFollow
        ? followPromptButtonLabel.trim() || "ya te sigo"
        : "",
      followUpEnabled,
      ...
      if (data.success) {
        ...
        setError(
          firstField
            ...
            : data.error ?? "No se pudo guardar la campaña"
        );
  ...
  return (
    <div className="space-y-6">
      {importQueue && (
        <div className="rounded border border-accent/30 bg-accent/5 px-4 py-3 text-sm">
          ...
          <span className="text-muted">
            Los campos vienen prellenados desde tu CSV. Elige el reel, edita lo
            que quieras y guarda para cargar el siguiente, o toca Omitir si no
            quieres este.
          </span>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
        <div className="flex min-w-0 items-center gap-3">
          {mode === "edit" ? (
            ...
            <span className="text-sm text-muted">Nueva campaña</span>
          )}
        </div>
      ...
      <div className="space-y-8 min-w-0">
        ...
        <Section title="Cuando alguien comenta en">
          <Radio
          ...
          >
            la siguiente publicación o reel
          </Radio>
        </Section>

        <Section title="Y ese comentario incluye">
          <Radio
            checked={matchMode === "specific"}
            onSelect={() => setMatchMode("specific")}
          >
            una palabra o palabras específicas
          </Radio>
          {matchMode === "specific" && (
            <div className="space-y-1">
              <input
                ...
                className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
              />
              <p className="text-xs text-muted">Usa comas para separar las palabras</p>
            </div>
          )}
          <Radio
          ...
          <div className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2.5">
            <span className="text-sm text-foreground">
              también responder cuando alguien mande un DM con{" "}
              {matchMode === "any" ? "cualquier cosa" : "estas palabras"}
            </span>
            <Toggle
              on={dmTriggerEnabled}
          ...
          {dmTriggerEnabled && (
            <p className="text-xs text-muted">
              {matchMode === "any"
                ? "Cada DM a esta cuenta recibe la respuesta de abajo; úsalo con cuidado."
                : "Un DM que contenga cualquiera de estas palabras recibe la misma respuesta, sin necesidad de comentario."}
            </p>
          )}
          <div className="flex items-center justify-between rounded-lg border border-border px-3 py-2.5">
            <span className="text-sm text-foreground">
              responder a sus comentarios debajo de la publicación
            </span>
            <Toggle
          ...
          {publicReplyEnabled && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted">
                Se elige una al azar cada vez, para que las respuestas no se vean
                idénticas.
              </p>
            </div>
          )}
        ...
        <Section title="Recibirán">
          <div className="rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-foreground">un DM de bienvenida</span>
              <Toggle
                on={openingDmEnabled}
            ...
            {openingDmEnabled && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="¡Hola! Me da mucho gusto que estés aquí 😊"
                  rows={3}
            ...
            {requireFollow && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="un favor rápido antes de mandarte el enlace. no gano nada con esto, es gratis. si quieres apoyarme, solo no me dejes de seguir después, y dale una estrella al repo en github si te sirve. toca el botón cuando ya me sigas y te lo mando"
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
                ...
                />
                <p className="text-xs text-muted">
                  Enviamos el enlace solo después de que tocan el botón e
                  Instagram confirma que te siguen. Si no se puede verificar, lo
                  enviamos de todos modos.
                </p>
              </div>
            )}
          </div>
        ...
        <Section title="Y después, recibirán">
          <div className="rounded-lg border border-border p-3 space-y-2">
            ...
            {linkOpen ? (
              <div className="space-y-2">
                <input
                  ...
                  placeholder="https://tuenlace.com/oferta"
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
                />
                <input
                  value={linkButtonLabel}
                  onChange={(e) => setLinkButtonLabel(e.target.value)}
                  placeholder="Texto del botón (ej. Abrir enlace)"
                  maxLength={20}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
                />
                {secondLinkOpen ? (
                  <div className="space-y-2 border-t border-border pt-2">
                    <input
                      ...
                      placeholder="https://tuenlace.com/segundo"
                      className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
                    />
                    <input
            ...
            <p className="text-xs text-muted">
              {"{link}"} inserta el enlace rastreado; {"{username}"} personaliza.
            </p>
          </div>
          <div className="mt-3 rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-foreground">
                un mensaje de agradecimiento de seguimiento
              </span>
              <Toggle
                on={followUpEnabled}
            ...
            {followUpEnabled && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Por cierto, solo quería agradecerte por seguirme, aprecio mucho el apoyo 🙌"
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
                <div className="flex flex-wrap items-center gap-2 text-sm text-foreground">
                  <span className="text-xs text-muted">Envíalo</span>
                  <input
                    type="number"
                    min={0}
                    max={1440}
                ...
                <p className="text-xs text-muted">
                  {followUpDelayMinutes > 0
                    ? `Enviado ${followUpDelayMinutes} min después de que dan clic.`
                    : "Enviado justo después de que dan clic."}
                  {" {username}"} lo personaliza. Máximo 24 horas, para mantenerse
                  dentro de la ventana de mensajería de Instagram.
                </p>
              </div>
            )}
          </div>
      ...
      <div>
        ...
        <div className="flex min-w-0 justify-center lg:sticky lg:top-6 lg:block">
          <CampaignPreview
            ...
            followPromptButtonLabel={followPromptButtonLabel || "ya te sigo"}

  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted mt-1">
            {data.requestedCount === "all" ? "Histórico" : "Reciente"}:{" "}
            {totals.posts} {totals.posts === 1 ? "publicación" : "publicaciones"} de @
      ...
      {!insightsAvailable && (
        <div className="panel rounded p-4 border border-border">
          <p className="text-sm text-foreground">
            Las vistas, el alcance, los guardados y las veces compartido necesitan el permiso de estadísticas.
          </p>
          <p className="text-sm text-muted mt-1">
            Reconecta tu cuenta para otorgarlo; mientras tanto se muestran los me
            gusta y los comentarios.
          </p>
          <a
            href="/api/instagram/connect"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        <StatCard label="Vistas" value={formatNumber(totals.views)} />
        <StatCard label="Alcance" value={formatNumber(totals.reach)} />
        <StatCard label="Me gusta" value={formatNumber(totals.likes)} />
        <StatCard label="Comentarios" value={formatNumber(totals.comments)} />
        <StatCard label="Guardados" value={formatNumber(totals.saved)} />
        <StatCard label="Compartidos" value={formatNumber(totals.shares)} />
      </div>
      ...
      <div className="panel rounded p-4 sm:p-6">
        <h2 className="text-sm font-semibold text-foreground mb-4">Publicaciones</h2>
        {posts.length === 0 ? (
          <p className="text-sm text-muted py-8 text-center">No se encontraron publicaciones</p>
        ) : (
            ...
            <table className="w-full min-w-[720px] text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wide text-zinc-500 border-b border-border">
                  ...
                  <th className="py-2 px-3 font-medium text-right">Vistas</th>
                  <th className="py-2 px-3 font-medium text-right">Alcance</th>
                  <th className="py-2 px-3 font-medium text-right">Me gusta</th>
                  <th className="py-2 px-3 font-medium text-right">Comentarios</th>
                  <th className="py-2 px-3 font-medium text-right">Guardados</th>
                  <th className="py-2 px-3 font-medium text-right">Compartidos</th>
                  <th className="py-2 pl-3 font-medium text-right">Fecha</th>
                </tr>
              </thead>
              <tbody>
                {posts.map((p) => (
                  ...
                  >
                    <td className="py-3 pr-4 max-w-xs">
                      {p.permalink ? (
                        ...
                        >
                          {p.caption || `Publicación de ${p.mediaType}`}
                        </a>
                      ) : (

...
export default function PostPicker({
  ...
  useEffect(() => {
    ...
    fetch(`/api/instagram/posts${params.size ? `?${params}` : ""}`)
      ...
      .catch(() => {
        if (!cancelled && !cached.data) setError("No se pudieron cargar las publicaciones");
      })
      .finally(() => {
  ...
  if (error) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-xs text-zinc-500 mt-1">Primero conecta tu cuenta de Instagram</p>
      </div>
    );
  }
  ...
  if (posts.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-muted">No se encontraron publicaciones</p>
      </div>
    );
  }
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <input
          ...
          placeholder="Busca tus publicaciones por descripción…"
      ...
      {visible.length === 0 ? (
        ...
        <>
          ...
          <button
            ...
            title={isUsed ? `Ya la usa "${usedByName}"` : undefined}
            className={`

    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            Salud, colas, fallos de webhook, eventos de facturación y alertas del worker.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3">
        <div className="panel rounded p-4 sm:p-5">
          ...
          >
            {data?.workerHealth.healthy ? "Saludable" : "Necesita atención"}
          </p>
          <p className="mt-2 text-xs text-muted">
            {workerAgeSeconds == null
              ? "No se encontró latido"
              : `Último latido hace ${workerAgeSeconds}s`}
          </p>
        </div>
      ...
      <Section title="Alertas recientes del worker">
        {data?.workerAlerts.length ? (
          ...
          <EmptyState label="No hay alertas del worker registradas." />
        )}
      </Section>

      <div className="grid gap-6 lg:grid-cols-2">
        <Section title="Fallos y omisiones de DMs de campañas">
          {data?.dmFailures.length ? (
            ...
            <EmptyState label="No hay fallos ni omisiones de DMs." />
          )}
        </Section>

        <Section title="Fallos de webhook">
          {data?.webhookFailures.length ? (
              ...
              {data.webhookFailures.map((event) => (
                <div key={event.id} className="border-b border-border pb-3 last:border-0">
                  <p className="text-sm font-semibold text-foreground">
                    {event.object ?? "Webhook de Instagram"}
                  </p>
            ...
            <EmptyState label="No hay eventos de webhook fallidos." />
          )}
        </Section>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <Section title="Fallos al renovar el token">
          {data?.tokenRefreshFailures.length ? (
            ...
            <EmptyState label="No hay fallos al renovar el token." />
          )}
        </Section>

      </div>

      <Section title="Línea de tiempo de eventos operativos">
        {data?.operationalEvents.length ? (
          ...
          <EmptyState label="No hay eventos operativos registrados." />
        )}
      </Section>
    </div>
  );

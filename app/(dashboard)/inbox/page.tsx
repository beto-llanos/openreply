    ...
    async (silent: boolean) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setConvError(data.error ?? "No se pudieron cargar las conversaciones");
        }
      } catch {
        if (!silent) setConvError("No se pudieron cargar las conversaciones");
      } finally {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-4">
        <h1 className="text-lg font-semibold text-foreground">Bandeja</h1>
        {accounts.length > 1 && (
          <AccountSelect
            accounts={accounts}
        ...
        >
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {convLoading ? (
              ...
              <p className="px-4 py-6 text-sm text-muted">Aún no hay conversaciones.</p>
            ) : (
              conversations.map((c) => {
                  ...
                  >
                    <div className="flex items-baseline justify-between gap-2">
                      <span className="truncate text-sm font-medium text-foreground">
                        @{c.contact.username ?? "desconocido"}
                      </span>
            ...
            <>
              <div className="flex shrink-0 items-center gap-2 border-b border-border px-4 py-3 text-sm font-semibold text-foreground">
                ...
                <span className="truncate">
                  @{active.contact.username ?? "desconocido"}
                </span>
              </div>
              ...
              <div ref={scrollRef} className="min-h-0 flex-1 space-y-2 overflow-y-auto p-4">
                {threadLoading && messages.length === 0 ? (
                  ...
                  <p className="text-sm text-muted">No hay mensajes.</p>
                ) : (
                  messages.map((m) => (
                    <div
                      key={m.id}
